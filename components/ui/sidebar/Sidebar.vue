...
<template>
  ...
  <div
    v-else class="group peer max-md:hidden flex-none"
    :data-state="state"
    :data-collapsible="state === 'collapsed' ? collapsible : ''"
    :data-variant="variant"
    :data-side="side"
  >
    <!-- Spacer that reserves flex space equal to the sidebar width -->
    <div
      class="duration-200 relative h-svh bg-transparent transition-[width] ease-linear shrink-0"
      :style="{
        width: state === 'collapsed'
          ? (collapsible === 'icon' ? SIDEBAR_WIDTH_ICON : '0px')
          : SIDEBAR_WIDTH,
      }"
    />
    <div
