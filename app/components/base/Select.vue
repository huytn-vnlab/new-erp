...
// Vue's automatic $attrs fallthrough lands on SelectRoot (a non-visual context
// provider), not the visible SelectTrigger button — so a caller's `:class` (e.g.
// validation-error ring) would silently never render. Disable auto-inherit and
// forward attrs explicitly onto SelectTrigger instead.
defineOptions({ inheritAttrs: false })

const toStr = (v: string | number | undefined) => v != null ? String(v) : ''

/** Computed label shown in trigger — works in SSR/test without portal items mounted */
const displayLabel = computed(() => {
  if (props.modelValue == null || props.modelValue === '') return ''
  return props.options.find(o => String(o.value) === String(props.modelValue))?.label ?? ''
})
...
<template>
  <SelectRoot :model-value="toStr(modelValue)" @update:model-value="onSelect($event as string)">
    <SelectTrigger
      v-bind="$attrs"
    ...
    <SelectPortal>
      ...
      >
        <SelectViewport class="max-h-60 overflow-y-auto scrollbar-thin py-1">
          <SelectItem
            v-for="o in options" :key="String(o.value)" :value="String(o.value)"
